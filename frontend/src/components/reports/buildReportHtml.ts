...
function barChart(data: { name: string; value: number }[], width: number, height: number, formatter: (v: number) => string): string {
  ...
  data.forEach((d, i) => {
    ...
    svg += `<text x="${labelW - 6}" y="${y + barH / 2 + 4}" text-anchor="end" font-size="9" fill="#6C757D" font-family="monospace">${d.name}</text>`;
    svg += `<rect x="${labelW}" y="${y + 2}" width="${Math.max(bw, 2)}" height="${barH - 4}" rx="3" fill="#0F5132"/>`;
    svg += `<text x="${labelW + bw + 6}" y="${y + barH / 2 + 4}" font-size="9" fill="#6C757D" font-family="monospace">${formatter(d.value)}</text>`;
  });
  return svg;
}
...
function lineChart(data: { year: string; count: number }[], width: number, height: number): string {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    grid += `<line x1="${pad.left}" y1="${y}" x2="${width - pad.right}" y2="${y}" stroke="#DEE2E6" stroke-width="1"/>`;
    grid += `<text x="${pad.left - 6}" y="${y + 4}" text-anchor="end" font-size="9" fill="#6C757D">${val}</text>`;
  }
  ...
  let dots = '';
  points.forEach(p => {
    dots += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="#0F5132" stroke="#fff" stroke-width="1.5"/>`;
  });
  let labels = '';
  data.forEach(d => {
    const x = pad.left + ((parseInt(d.year) - minYear) / yearRange) * cw;
    labels += `<text x="${x}" y="${height - 5}" text-anchor="middle" font-size="9" fill="#6C757D">${d.year}</text>`;
  });
  return `${grid}<path d="${lineD}" fill="none" stroke="#0F5132" stroke-width="2.5"/>${dots}${labels}`;
}

function legendHtml(items: { name: string; value: number; color: string }[]): string {
  return items.map(d => `<span style="display:inline-flex;align-items:center;gap:4px;font-size:9px;color:#6C757D"><span style="width:8px;height:8px;border-radius:50%;background:${d.color}"></span>${d.name}: <strong>${d.value}</strong></span>`).join('');
}
...
function pageFooter(): string {
  return '<div class="pftr"><span>CGASI.SE — Coordenação Geral de Articulação e Supervisão Institucional / MAPA</span><span>Documento Confidencial</span></div>';
}
...
export function buildReportHtml(demands: Demand[], filters: { uf: string; status: string; priority: string; ano: string }, userLabel: string): string {
  ...
  /* Icons as SVG */
  const iconBrain = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0F5132" stroke-width="2"><path d="M12 2a4 4 0 0 1 4 4c0 1.1-.4 2.1-1 2.8V12l-3 2-3-2V8.8c-.6-.7-1-1.7-1-2.8a4 4 0 0 1 4-4z"/><path d="M9 12v6"/><path d="M15 12v6"/></svg>';
  const iconLightbulb = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F4B400" stroke-width="2"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M15.09 14c.18-.98.65-1.74 1.41-2.5A4.65 4.65 0 0 0 18 8 6 6 0 0 0 6 8c0 1.49.45 2.77 1.5 3.5.76.76 1.23 1.52 1.41 2.5"/></svg>';
  const iconBarChart = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0F5132" stroke-width="2"><rect x="3" y="12" width="4" height="9"/><rect x="10" y="7" width="4" height="14"/><rect x="17" y="3" width="4" height="18"/></svg>';
  const iconTarget = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#20C997" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>';
...
.cvr-bd{flex:1;padding:20mm 20mm 10mm;display:flex;flex-direction:column}
.cvr-lg{display:flex;align-items:center;gap:16px;margin-bottom:32px}
.cvr-lg-logo{height:80px;width:auto;border-radius:12px;box-shadow:0 10px 15px -3px rgba(15,81,50,.2)}
...
.cvr-bdg{font-size:10px;font-weight:700;color:#0F5132;text-transform:uppercase;letter-spacing:.25em;background:#bdf2e2;padding:6px 12px;border-radius:999px;display:inline-block;width:fit-content;margin-bottom:12px}
.cvr-tit{font-size:32px;font-weight:900;color:#212529;line-height:1.1;letter-spacing:-.02em;margin-bottom:8px}
.cvr-sub{font-size:22px;color:#495057;font-weight:300;margin-bottom:24px}
.cvr-info{border-top:1px solid #a8e8d0;padding-top:20px;max-width:420px}
...
.phdr-logo{height:28px;width:auto;border-radius:4px}
...
.scr-btn button{padding:10px 32px;background:linear-gradient(to right,#0F5132,#198754);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer}
...
<div class="cvr">
  <div class="cvr-bd">
    ...
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center">
      ...
      <p class="cvr-sub">Análise quantitativa e qualitativa das solicitações registradas no período</p>
      <div class="cvr-info">
  ...
  <div class="cvr-ft">
    <span>CGASI.SE — Coordenação Geral de Articulação e Supervisão Institucional / MAPA</span>
...
<div class="pg">
  ...
  <div class="pg-bd">
    <h2 style="font-size:18px;font-weight:900;color:#212529;margin-bottom:20px">Indicadores Gerenciais</h2>
    <div class="kpi-g">
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#d6f7ed;color:#0F5132"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></div>
        <div><p class="kpi-l">Total de Demandas</p><p class="kpi-v">${m.total}</p></div>
      </div>
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#d6f7ed;color:#198754"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg></div>
        <div><p class="kpi-l">Valor Solicitado</p><p class="kpi-v">${fc(m.totalValue)}</p></div>
      </div>
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#d6f7ed;color:#20C997"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></div>
        <div><p class="kpi-l">Valor Aprovado</p><p class="kpi-v">${fc(m.approvedValue)}</p></div>
      </div>
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#d6f7ed;color:#20C997"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 15H8.5C6.01 15 5 13.48 5 11.5 5 9.52 6.01 8 8.5 8H12"/><path d="M12 8h3.5c2.49 0 3.5 1.52 3.5 3.5s-1.01 3.5-3.5 3.5H12"/><circle cx="12" cy="15" r="2"/><path d="M12 2v2m0 20v-2"/><path d="M22 12h-2M4 12H2"/></svg></div>
        <div><p class="kpi-l">Valor Concluído</p><p class="kpi-v">${fc(m.concludedValue)}</p></div>
      </div>
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#fce4ec;color:#dc3545"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div>
        <div><p class="kpi-l">Valor Rejeitado</p><p class="kpi-v">${fc(m.rejectedValue)}</p></div>
      </div>
      <div class="kpi-c">
        <div class="kpi-ic" style="background:#fef3c7;color:#F4B400"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg></div>
...
<div class="pg">
  ...
  <div class="pg-bd">
    ${sectionTitle('Análise Executiva', '#0F5132', iconBrain)}
    <div class="anl">
...
<div class="pg">
  ...
  <div class="pg-bd">
    ${sectionTitle('Análise Gráfica', '#0F5132', iconBarChart)}
    <div class="ch-g">
      <div class="ch-b">
    ...
    <div class="ch-g">
      <div class="ch-b">
        ...
        <div style="display:flex;justify-content:center;gap:8px;flex-wrap:wrap;margin-top:8px">
          ${ufPie.map(d => `<span style="font-size:9px;color:#6C757D;font-family:monospace;font-weight:700">${d.name}</span>`).join('')}
        </div>
      </div>
      <div class="ch-b">
...
<div class="pg">
  ...
  <div class="pg-bd">
    <h2 style="font-size:18px;font-weight:900;color:#212529;margin-bottom:6px">Tabela Executiva</h2>
    <p style="font-size:11px;color:#6C757D;margin-bottom:14px">${demands.length} demandas</p>
    <div class="tb-w">
      <table class="tb">
        ...
        <tbody>
          ${[...demands].sort((a, b) => {
            ...
            const stColor = SC[d.status] || '#6C757D';
            const prColor = PC[d.priority] || '#6C757D';
            return `<tr>
...
<div class="pg">
  ...
  <div class="pg-bd">
    ${sectionTitle('Recomendações Estratégicas', '#0F5132', iconTarget)}
    <div class="rec-l">
    ...
    <div class="rec-concl">
      ...
      <p style="font-size:9px;color:#6C757D">Este documento é confidencial e destinado ao uso interno.</p>
    </div>
  </div>
  ${pageFooter()}
</div>
