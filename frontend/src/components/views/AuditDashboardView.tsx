...
export default function AuditDashboardView() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Auditoria"
        subtitle="Rastreabilidade das aÃ§Ãµes e histÃ³rico do sistema"
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
            ...
            <Kpi icon={<Users size={20} />} label="UsuÃ¡rios Ativos" value={String(stats.active_users)} accent="green" />
            <Kpi icon={<LogIn size={20} />} label="SessÃµes Ativas" value={String(stats.active_sessions)} accent="blue" />
            <Kpi icon={<PlusCircle size={20} />} label="Demandas Criadas" value={String(stats.demands_created)} accent="green" />
            <Kpi icon={<Trash2 size={20} />} label="Demandas ExcluÃ­das" value={String(stats.demands_deleted)} accent="rose" />
            <Kpi icon={<ShieldOff size={20} />} label="Alt. PermissÃµes" value={String(stats.permission_changes)} accent="green" />
            <Kpi icon={<UserCog size={20} />} label="Alt. UsuÃ¡rios" value={String(stats.user_changes)} accent="amber" />
          </div>

          <AuditTrail />
        </>
      ) : (
        <p className="text-sm text-slate-400 italic">NÃ£o foi possÃ­vel carregar os dados.</p>
      )}
    </div>
  );
}
